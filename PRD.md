...
> **版本:** 3.0
> **日期:** 2026-05-26
> **狀態:** 領収書を税率別合計に集約（明細逐行出力を廃止）
>
> ### v3.0 変更点 (2026-05-26)
> - **領収書の税率別集約**: 領収書フォルダの書類について、品目を逐行出力する従来方式を廃止し、8%/10% の税率別に合計1行ずつ出力する方式に変更（5/25 会議で確定）
>   - 複数税率が混在する場合は税率ごとに1行（**総合計行は出さない**）、単一税率のみの場合は合計1行
>   - 対象は領収書フォルダ（`DocType.RECEIPT`）の **PDF / 画像の両方**。請求書・給与明細は従来通り逐行出力
>   - 集約行の借方勘定科目は税率グループ内で金額最大の品目の科目を採用（**暫定仕様**、`AGGREGATED_DEBIT_ACCOUNT_STRATEGY` 定数で切替可能。最終ルールは顧客確認待ち）
>   - 摘要は「{店名} - {税率}%対象」形式（例: `ファミマ - 10%対象`）。税率ラベルのみを集約側で生成し、店名は `sheets_output` が前置（二重回避）
>   - `coerce_tax_rate` で tax_rate を正規化（null / 文字列"0.08" / bool → 既定10%）し、不正値でのクラッシュ・税区分との不整合・分組ずれを防止
>   - 集約ロジックは `receipt_aggregation.py` に分離（外部依存なしの純粋関数）、単体テスト17件（`python3 -m unittest test_receipt_aggregation`）
>
> ### v2.9 変更点 (2026-04-24)
> - **OCR エンジン v3 アップグレード**: PaddleOCR 2.10.0 → 3.5.0、PaddlePaddle 2.6.2 → 3.3.1
>   - Apple Silicon (ARM64) で v2 が libphi.dylib sgemm 推論中に SIGSEGV → `google::FailureSignalHandler` 無限ループに陥る問題を解消
>   - PP-OCRv5_server モデルで日本語認識精度向上、文書方向検出 / テキストライン方向 / UVDoc の統合パイプラインを採用
>   - `requirements.txt`: `paddleocr>=3.0` / `paddlepaddle>=3.0` に変更（従来の `<3.0` 上限を撤廃）
> - **生成 AI モデル更新**: gemini-2.0-flash → gemini-2.5-flash（個別モデルのクォータ枯渇（429 Resource exhausted）回避、最新 Flash モデル活用）
> - **API 接続リトライ**: 一時的ネットワーク切断 / `RemoteDisconnected` / `ServiceUnavailable` / `DeadlineExceeded` / `ResourceExhausted` に対し指数バックオフ 4 回リトライ（1s → 4s → 10s → 30s）
> - **ページ単位の例外捕捉**: 1 ページの処理エラーで PDF 全体が中断する不具合を修正（エラーページは認識不能として記録し、次ページへ続行、ファイルは正常に `processed/` へ移動）
> - **レジューム機能**: `local_test.py --start-page N` で指定ページから再開、`--only-file <substr>` で特定ファイルのみ処理
> - **書式伝播バグ修正**: Google Sheets の `append_row` が直前行の書式を継承する挙動により、1 行の異常ハイライト（B 列日付空 / H 列 T 番号空 等）が下方の全後続行に伝播していた問題を修正。新規行書き込み後に A:AB 範囲を白色でリセットしてから異常色を被せる方式に変更
> - **テスト検証**: Mac ARM64 で 25 頁 PDF（2025年領収書、89 行、成功率 100%）+ 478 頁 PDF（2023年領収書、481 件抽出、失敗ページ 2）+ 280 頁 PDF（部分実行）を処理完了、計 1,897 行書き込み
>
> ### v2.8 変更点 (2026-04-04)
> - **納付書日付精度改善**: 「納期限」「提出期限」の日付をスキップし、「発行日」「取引日」を優先抽出（簡体字OCR誤読「纳期限」も対応）
> - **パンフレット過濾強化**: 長文(500字超) + 複数説明キーワード(制度/処分の流れ/とは？等) → 財務用語を含んでいても自動除外（放置違反金制度チラシ等）
> - **手書きメモ完全除外**: 閾値30→60字に拡大、領収書構造キーワード(領収/合計等)がなければ金額の有無に関わらず除外（④ENITY手書きメモ5/5過濾達成）
> - **半角「とは?」対応**: パンフレット検出キーワードに半角疑問符バリアントを追加
> - **テスト検証**: 6PDF+1画像(淡色感熱紙)の完全テスト完了、22ページ正確過濾、正常領収書の誤過濾0件
>
> ### v2.7 変更点 (2026-04-03)
> - **認識不能ページ対応**: 全部認識不能→整行赤ハイライト占位行、部分認識(日付のみ等)→欠損セル赤ハイライト
> - **不要ページ除外拡充**: 封筒(既存)+送付状+挨拶状+裏面メモ+説明書パンフレット→自動スキップ
> - **領収証(合計のみ)対応**: 単一品目の合計行をsubtotalフィルタから除外（LAWSONレシート等）
> - **貸方科目兜底**: 借方に未払金/仮受金等の貸方専用科目が出現→未確定勘定に自動置換
> - **OCR日付精度改善**:
>   - 宣伝日付(終了/有効期限/ポイント等)をスキップ、取引日を優先抽出
>   - 複数書類ページ: Geminiの書類ごと日付を維持（OCR上書きしない）
>   - Geminiが有効な近年日付を持つ場合、OCRの未来年日付で上書きしない
>   - ATM明細「07-MM-DD」形式（令和YY）の日付パターン追加
> - **Gemini prompt改善**: 飲食店/百貨店催事場/サロン→接待交際費、ホテルフロント→接待交際費(地代家賃にしない)、乗車券→items含める
> - **科目兜底追加**: Mister Donut/Hotto Motto/FamilyMart/Diner等→接待交際費
> - **PDF間分割線**: 太黒線で区切り、取引Noを各PDFごとにリセット
> - **重複検出**: 一時実装後、誤検出が多いため無効化（社長の意図確認後に再実装予定）
>
> ### v2.6 変更点 (2026-04-02)
...
| **v2.7** | **認識不能ページ対応、不要ページ除外拡充、OCR日付精度改善** | ✅ 完成 |
| **v2.8** | **納付書日付精度、パンフレット強化過濾、手書きメモ完全除外** | ✅ 完成 |
| **v2.6** | **客戶本番稼働、GAS日次バックアップ、main マージ** | ✅ 完成 |
| **v2.9** | **OCR v3 移行、API リトライ、ページ容錯、レジューム、Sheets 書式伝播バグ修正** | ✅ 完成 |
...
*本文檔反映 Super Scaner v2.9 OCR v3 移行 + 障害耐性強化後の狀態，截至 2026-04-24。*
