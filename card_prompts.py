...
_ROW_FIELDS_COMMON = {
    ...
    "date": ('"2026/04/10"',
             "利用日 YYYY/MM/DD。**年の決め方は上の【最重要】に従う**。"
             "そこでも決められなければ null（当て推量で埋めない）"),
    "amount": ("630",
...
- **他ページの明細行を推測で補わないでください。** 1 つの PDF に別会社・別明細書の
  カード明細が混在することがあります。**このページに印字されていない明細行を
  作らないでください。**（禁じているのは行を作ることです。下の年の確定は行います）
- **日付の年は券面から確定してください。** 明細行の日付欄は「12月18日」のように
  月日だけのことがあります。そのときはこのページ上部の**明細書作成日から年を
  決めてください**。作成日が 2023年1月19日なら 12月は 2022年、1月は 2023年です
  （明細書は作成日以前の利用を載せるため）。これは推測ではなく券面の記載から
  導く作業です。作成日も読めないときだけ date を null にし、month_day に
  券面の逐語を残してください。
...
- **「上記に含まれる課税取引の明細」の区画の行を rows に入れないでください。**
  見出しが示すとおり、それらは**上で既に計上済みの行の再掲**であり、消費税額を
  示すための表示です（インボイス対応）。新規の取引ではないので、入れると
  二重計上になります。**この区画が在るときは、その見出しの逐語を必ず
  sections に入れてください**（見出しだけ報告し、行は入れない）。
