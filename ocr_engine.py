    ...
    "documents": [
        {
            ...
            "invoice_num": "適格請求書発行事業者登録番号 (T+数字13桁, 例: T1234567890123)。「登録番号」「Registration Number」として記載されているもののみ。以下は含めない: 領収書No, 取引番号, 伝票番号, レシート番号, 整理番号, 注文番号。ハイフンは除去。見つからなければ必ず空文字を返す",
...
【tax_rate の判定基準 — 軽減税率 vs 標準税率】
重要: 日本の消費税は品目ごとに税率が異なります。正確に判定してください。

■ 0.08（軽減税率 8%）:
  - スーパー・コンビニ・ドラッグストアで購入した飲食料品（食品・飲料・菓子等）
  - 精肉店・鮮魚店・酒屋等の小売店で購入した食品（テイクアウト・持ち帰り）
  - レシートに「※」「軽」「8%」マークがある品目
  - 新聞（定期購読）

■ 0.10（標準税率 10%）:
  - レストラン・居酒屋・カフェ・バー等での外食（店内飲食）
  - 酒類（ビール、ワイン、日本酒、焼酎等）
  - 日用品・雑貨・文房具・衣料品等の非食品
  - サービス料（タクシー、ホテル、レンタカー、印刷等）
  - ガソリン・高速道路料金

■ 0（非課税・対象外）:
  - 銀行振込本体(bank_transfer)

判定のコツ:
- レシートに税率が印字されている場合はそれに従う（最優先）
- 同一レシート内でも品目ごとに税率が異なることがある（食品8%+酒類10%等）
- 迷った場合: 飲食料品（酒類除く）→ 0.08、それ以外 → 0.10
